 ...
 */
async function fetchUsdtToTomanFromNobitex(): Promise<number | null> {
  ...
  try {
    ...
    const data = (await response.json()) as {
      status?: string;
      stats?: Record<string, { latest?: string | number } | undefined>;
    };
    if (data.status !== "ok") return null;

    const latestRaw = data.stats?.["usdt-rls"]?.latest;
    const usdtRial = typeof latestRaw === "string" ? Number(latestRaw) : latestRaw;
    if (!Number.isFinite(usdtRial) || !usdtRial || usdtRial <= 0) return null;
...
async function fetchFromNobitexUsdt(): Promise<RateApiResult | null> {
  const usdtToman = await fetchUsdtToTomanFromNobitex();
  if (usdtToman === null) return null;
 ...
 */
function todayJalaliLabel(): string {
  try {
...
function renderTodayDate(els: Elements): void {
